...
el("tp_searchTrips").oninput = (e) => {
  ...
  function renderSettlement(trip) {
    ...
    trip.expenses.forEach((e) => {
      ...
      e.sharedWith.forEach((pid) => {
        totals[pid] -= perPerson;
      });
    ...
    trip.persons.forEach((p) => {
      ...
      if (bal < 0) debtors.push({ name: p.name, balance: -bal });
      else if (bal > 0) creditors.push({ name: p.name, balance: bal });
    });
    ...
    while (i < debtors.length && j < creditors.length) {
      ...
      payments.push({ from: debtor.name, to: creditor.name, amount: amt });
      debtor.balance -= amt;
    ...
    el("tp_settlementList").innerHTML = `<p class="text-xs text-gray-400 mb-2">Total Trip Amount: ₹${totalAmount.toFixed(
      2
    )} | Per Head: ₹${perHead.toFixed(2)}</p>${payments.length ? payments
      .map((p) => `<div class="flex justify-between text-sm text-gray-300"><span>${p.from} ➡ ${p.to}</span><span>₹${p.amount.toFixed(
        2
      )}</span></div>`)
      .join("") : `<div class="text-xs text-gray-400">All settled ✅</div>`}`;
  }
