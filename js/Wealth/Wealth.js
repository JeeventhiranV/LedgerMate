...
const ASSET_CATEGORIES = {
  Equity:        { icon: '📈', color: '#3b82f6', types: ['STOCK','SIP_EQUITY','MF_EQUITY'] },
...
// ─── Get category for an asset/investment ────────────────────
...
// ─── Get current value of any investment/asset ───────────────
function getAssetCurrentValue(inv) {
  if (!inv) return 0;
  const t = (inv.type || '').toUpperCase();
  if (['GOLD','SILVER','PHYSICAL','COMMODITY'].includes(t)) {
    const qty = toNum(inv.qty || inv.stockQty || 0);
    const cur = toNum(inv.currentPrice || inv.ltp || inv.stockCurrentPrice || 0);
    const buy = toNum(inv.buyPrice || inv.avgCost || inv.stockBuyPrice || 0);
    return qty * (cur > 0 ? cur : buy);
  }
  if (t === 'REAL_ESTATE') {
    return toNum(inv.currentValue || inv.buyPrice || 0);
  }
  if (t === 'STOCK') {
    const qty = toNum(inv.stockQty || inv.qty || 0);
    const cur = toNum(inv.stockCurrentPrice || inv.currentPrice || inv.stockBuyPrice || 0);
    return qty * cur;
  }
  // For FD/RD/SIP use calculateMaturity if available
  if (typeof calculateMaturity === 'function') {
    const mat = calculateMaturity(inv);
    return mat > 0 ? mat : toNum(inv.principal || inv.sipMonthly || 0);
  }
  return toNum(inv.principal || inv.currentValue || inv.amount || 0);
}

// ─── Get invested amount for asset ───────────────────────────
function getAssetInvestedAmount(inv) {
  if (!inv) return 0;
  const t = (inv.type || '').toUpperCase();
  if (['GOLD','SILVER','PHYSICAL','COMMODITY'].includes(t)) {
    return toNum(inv.qty || inv.stockQty || 0) * toNum(inv.buyPrice || inv.avgCost || inv.stockBuyPrice || 0);
  }
  if (t === 'REAL_ESTATE') return toNum(inv.buyPrice || inv.principal || 0);
  if (t === 'STOCK') return toNum(inv.stockQty || 0) * toNum(inv.stockBuyPrice || 0);
  return toNum(inv.principal || inv.amount || 0);
}

// ─── Current wealth state ─────────────────────────────────────
function getTotalAssets() {
  const invTotal = (state.investments || []).reduce((sum, inv) => sum + getAssetCurrentValue(inv), 0);
  return invTotal;
}
...
function showWealthPage() {
  ...
  const tabs = ['assets','liabilities','networth','allocation'];
  ...
  page.innerHTML = `
    <div class="page-header fade-up fade-up-1">
      ...
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
        <button class="btn-submit" style="width:auto;padding:9px 18px;margin:0;font-size:13px;" onclick="openAddAssetModal()">+ Add Asset</button>
        <button class="btn-secondary" style="padding:9px 14px;font-size:13px;" onclick="exportWealthJSON()">📤 Export</button>
      </div>
    </div>
...
function switchWealthTab(tab) {
  ...
  page.querySelectorAll('.wealth-tab').forEach(b => b.classList.toggle('active', b.textContent.toLowerCase().replace(' ','') === tab || b.getAttribute('onclick').includes(`'${tab}'`)));
...
function renderWealthAssets(container) {
    ...
    <div class="chart-card" style="margin-bottom:16px;">
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;text-align:center;">
        <div>
          ...
          <div class="kpi-value" style="color:var(--text);">${fmtINR(totalInvested)}</div>
        </div>
        <div>
          <div class="kpi-label">CURRENT VALUE</div>
          <div class="kpi-value" style="color:var(--teal);">${fmtINR(totalCurrent)}</div>
        </div>
        <div>
          <div class="kpi-label">P&amp;L</div>
          <div class="kpi-value" style="color:${totalPnL>=0?'var(--emerald)':'var(--rose)'};">
            ${totalPnL>=0?'+':''}${fmtINR(totalPnL)} <span style="font-size:11px;">(${pnlPct}%)</span>
          </div>
        </div>
      </div>
    </div>`;
  ...
  const rows = assets.map(a => {
    ...
    const qtyDisplay = ['GOLD','SILVER','PHYSICAL','COMMODITY','STOCK'].includes(t) ? qty.toFixed(2) : '—';
    ...
    return `
      <tr class="wealth-row" data-id="${a.id}">
        ...
        <td class="wealth-td-mono" style="color:${pnl>=0?'var(--emerald)':'var(--rose)'};">
          ${pnl>=0?'+':''}${fmtINR(pnl)}<br>
          <span style="font-size:10px;">${pnl>=0?'+':''}${invested>0?((pnl/invested)*100).toFixed(1):0}%</span>
        </td>
  ...
  container.innerHTML = `
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:16px;">
      <div class="kpi-card rose">
        ...
        <div class="kpi-value" style="color:var(--rose);">${fmtINR(totalOutstanding)}</div>
...
function renderWealthNetWorth(container) {
  ...
  const snapshotList = snapshots.length === 0
    ? `<div class="empty-state" style="padding:32px;">
        ...
        <div class="empty-state-sub">Snapshots record your net worth at a point in time so you can track growth over months.</div>
       </div>`
    : `<div class="tx-list">` + [...snapshots].reverse().map(s => {
        ...
        return `<div class="list-item">
          ...
          <div style="text-align:right;">
            ...
            ${snapChange!==0?`<div style="font-size:11px;color:${snapChange>=0?'var(--emerald)':'var(--rose)'};">${snapChange>=0?'+':''}${fmtINR(snapChange)}</div>`:''}
          </div>
  ...
  container.innerHTML = `
    <div class="chart-card" style="margin-bottom:16px;text-align:center;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-top:16px;padding-top:16px;border-top:1px solid var(--border);">
        <div>
          ...
          <div style="font-family:var(--font-m);font-size:18px;font-weight:600;color:var(--emerald);">${fmtINR(assets)}</div>
        </div>
        <div>
          <div class="kpi-label">LIABILITIES</div>
          <div style="font-family:var(--font-m);font-size:18px;font-weight:600;color:${liabilities>0?'var(--rose)':'var(--text-3)'};">${fmtINR(liabilities)}</div>
        </div>
      </div>
    </div>
    ...
    <div class="chart-card" style="margin-bottom:16px;">
      ...
      <div class="chart-wrap" style="height:180px;">
        <canvas id="nwHistoryChart"></canvas>
      </div>
    </div>` : ''}

    <div class="section-heading">
      <div class="section-title"><span class="dot" style="background:var(--teal)"></span>Snapshots</div>
      <button class="btn-submit" style="width:auto;padding:8px 16px;margin:0;font-size:12px;" onclick="takeNetWorthSnapshot()">📸 Take Snapshot</button>
    </div>
    <div class="tx-card">${snapshotList}</div>`;

  // Render history chart if data available
      ...
      new Chart(ctx, {
        ...
        options: {
          ...
          scales: {
            ...
            y: { grid: { color: 'rgba(255,255,255,0.04)' }, ticks: { font: { size: 10 }, callback: v => '₹' + (v >= 100000 ? (v/100000).toFixed(1)+'L' : v >= 1000 ? (v/1000).toFixed(0)+'K' : v) } }
          }
        }
      });
    }, 50);
...
async function ensureAllocationTargets() {
  if (!state.allocation_targets || state.allocation_targets.length === 0) {
    ...
    if (saved.length === 0) {
      // seed defaults
      for (const d of DEFAULT_ALLOCATION) {
        await put('allocation_targets', d);
      }
...
function _doRenderAllocation(container) {
  ...
  assets.forEach(a => {
    const cat = getAssetCategory(a);
    actual[cat] = (actual[cat] || 0) + getAssetCurrentValue(a);
  });

  const targetMap = {};
  targets.forEach(t => targetMap[t.category] = t.target);

  const targetTotal = targets.reduce((s,t)=>s+toNum(t.target),0);
  const allCategories = [...new Set([...targets.map(t=>t.category), ...Object.keys(ASSET_CATEGORIES)])];

  // Build target allocation bar
  const barSegments = targets.map(t => {
    const catInfo = ASSET_CATEGORIES[t.category] || { color: '#888' };
    return `<div style="flex:${t.target};background:${catInfo.color};height:100%;border-radius:0;" title="${t.category}: ${t.target}%"></div>`;
  }).join('');
  ...
  const rows = targets.map(t => {
    ...
    const gap       = toNum(actualPct) - t.target;
    ...
    return `
      <tr>
        ...
        <td class="wealth-td-mono">
          <span style="display:inline-block;padding:2px 8px;border-radius:99px;font-size:11px;background:${Math.abs(gap)<=5?'rgba(52,211,153,0.12)':gap<0?'rgba(96,165,250,0.12)':'rgba(251,113,133,0.12)'};color:${Math.abs(gap)<=5?'var(--emerald)':gap<0?'var(--blue)':'var(--rose)'};">
            ${gap>0?'+':''}${gap}%
          </span>
        </td>
        <td>${action}</td>
  ...
  const needsRebalancing = targets.some(t => {
    const actualPct = totalCurrent > 0 ? (actual[t.category]||0) / totalCurrent * 100 : 0;
    return Math.abs(actualPct - t.target) > 5;
  });
  ...
  container.innerHTML = `
    <div class="two-col" style="margin-bottom:16px;">
      <div class="chart-card">
        <div class="chart-card-title"><span style="color:var(--gold)">●</span> Target Allocation <span class="badge-pill" style="margin-left:8px;font-size:10px;background:var(--bg3);padding:2px 8px;border-radius:99px;">Default</span>
        ...
        <div style="display:flex;flex-wrap:wrap;gap:8px;">
          ${targets.map(t => {
            const catInfo = ASSET_CATEGORIES[t.category]||{color:'#888'};
            return `<div style="display:flex;align-items:center;gap:4px;font-size:11px;color:var(--text-2);">
              <span style="width:8px;height:8px;border-radius:50%;background:${catInfo.color};display:inline-block;"></span>${t.category} ${t.target}%</div>`;
          }).join('')}
        </div>
      </div>
      <div class="chart-card" style="display:flex;flex-direction:column;align-items:center;">
        ...
        ${totalCurrent > 0 ? `<div class="chart-wrap" style="height:160px;width:100%;max-width:220px;"><canvas id="allocDoughnut"></canvas></div>` 
          : `<div class="empty-state" style="padding:24px;"><div class="empty-state-icon">🥧</div><div class="empty-state-text">No assets yet</div></div>`}
      </div>
    </div>

    ${needsRebalancing ? `<div style="background:rgba(251,113,133,0.1);border:1px solid rgba(251,113,133,0.3);border-radius:10px;padding:10px 14px;margin-bottom:16px;font-size:13px;color:var(--rose);">⚠️ Needs rebalancing — some categories are off by &gt;5%</div>` : ''}

    <div class="chart-card" style="margin-bottom:16px;overflow-x:auto;">
      <div class="section-heading" style="margin-bottom:12px;">
        <div class="section-title"><span class="dot"></span>Target vs Actual</div>
      </div>
      <table class="wealth-table">
        <thead>
          <tr>
            <th>CATEGORY</th><th>ACTUAL</th><th>CUR VAL</th><th>TARGET</th><th>TGT VAL</th><th>GAP</th><th>ACTION</th>
          </tr>
        </thead>
        <tbody>${rows}</tbody>
      </table>
    ...
    <div class="chart-card" id="sipPlanContainer">
      ${sipItems.length === 0
        ? `<div class="empty-state" style="padding:24px;">
            ...
            <div class="empty-state-sub">Track your monthly investments across mutual funds, stocks, and other instruments.</div>
            <button class="btn-submit" style="width:auto;padding:10px 20px;margin-top:12px;" onclick="openAddSIPModal()">+ Add Instrument</button>
           </div>`
  ...
  if (totalCurrent > 0) {
    setTimeout(() => {
      ...
      new Chart(ctx, {
        ...
        data: {
          labels: chartLabels,
          datasets: [{ data: chartData, backgroundColor: chartColors, borderWidth: 2, borderColor: 'transparent' }]
        },
        options: {
          responsive: true, maintainAspectRatio: false, cutout: '65%',
          plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => ` ${ctx.label}: ${fmtINR(ctx.raw)}` } } }
        }
      });
    }, 50);
  }
...
function renderSIPList(sipItems) {
  ...
  return `
    <div style="margin-bottom:12px;font-size:12px;color:var(--text-3);">
      Total monthly investment: <strong style="color:var(--teal);">${fmtINR(totalMonthly)}</strong>
    </div>
    ${sipItems.map(item => `
      <div class="list-item" style="justify-content:space-between;">
        <div style="display:flex;align-items:center;gap:10px;">
          ...
          <div>
            ...
            <div class="list-item-sub">${item.category} · ${item.percentage||0}%</div>
          </div>
        </div>
        <div style="display:flex;align-items:center;gap:8px;">
...
function openAddAssetModal(prefill = {}) {
          ...
          <form id="addAssetForm" class="space-y-4">
            <div class="grid-2">
              <div>
                ...
                <input id="ast_name" class="form-input" placeholder="e.g. Gold, HDFC Bank" value="${prefill.name||''}" required />
              </div>
              <div>
                <label class="form-label">Type*</label>
                <select id="ast_type" class="form-input" onchange="toggleAssetFields()">
                  ...
                  <option value="SIP" ${prefill.type==='SIP'?'selected':''}>📊 SIP / Mutual Fund</option>
                  ...
                  <option value="BOND" ${prefill.type==='BOND'?'selected':''}>📜 Bond / Debenture</option>
            ...
            <div id="ast_qty_section" class="grid-2">
              <div>
                ...
                <input id="ast_qty" type="number" step="0.001" class="form-input" placeholder="e.g. 10 grams" value="${prefill.qty||''}" />
              </div>
              <div>
                <label class="form-label">Buy Price / Avg Cost</label>
                <input id="ast_buyPrice" type="number" step="0.01" class="form-input" placeholder="per unit" value="${prefill.buyPrice||''}" />
              </div>
            </div>
            ...
            <div id="ast_cur_section" class="grid-2">
              <div>
                ...
                <input id="ast_currentPrice" type="number" step="0.01" class="form-input" placeholder="per unit" value="${prefill.currentPrice||''}" />
              </div>
              <div>
                <label class="form-label">Sub-Type / Label</label>
                <input id="ast_subType" class="form-input" placeholder="e.g. Physical Gold" value="${prefill.subType||''}" />
              </div>
            </div>
            ...
            <div id="ast_principal_section" class="grid-2" style="display:none;">
              <div>
                ...
                <input id="ast_principal" type="number" step="0.01" class="form-input" placeholder="₹" value="${prefill.principal||''}" />
              </div>
              <div>
                <label class="form-label">Interest Rate (%)</label>
                <input id="ast_rate" type="number" step="0.01" class="form-input" placeholder="e.g. 7.5" value="${prefill.rate||''}" />
              </div>
            </div>
            ...
            <div id="ast_tenure_section" class="grid-2" style="display:none;">
              <div>
                ...
                <input id="ast_tenure" type="number" class="form-input" placeholder="e.g. 36" value="${prefill.tenureMonths||''}" />
              </div>
              <div>
            ...
            <div>
              ...
              <input id="ast_notes" class="form-input" placeholder="Any notes..." value="${prefill.notes||''}" />
            </div>
  ...
  const container = document.getElementById('wealthModals');
...
function getAssetCategoryFromType(type) {
  for (const [cat, info] of Object.entries(ASSET_CATEGORIES)) {
  ...
  if (['STOCK','SIP'].includes(type)) return 'Equity';
...
function toggleAssetFields() {
  ...
  if (debtTypes.includes(type)) {
    ...
    principalSection.style.display = '';
    tenureSection.style.display = '';
        ...
        <div class="modal-body">
          <form id="addLiabForm" class="space-y-4">
            <div>
              ...
              <input id="liab_name" class="form-input" placeholder="e.g. Home Loan - SBI" value="${prefill.name||''}" required />
            </div>
            <div class="grid-2">
              <div>
                <label class="form-label">Type*</label>
              ...
              <div>
                ...
                <input id="liab_outstanding" type="number" step="0.01" class="form-input" placeholder="₹" value="${prefill.outstanding||''}" required />
              </div>
            </div>
            <div class="grid-2">
              <div>
                <label class="form-label">Interest Rate (%)</label>
                <input id="liab_rate" type="number" step="0.01" class="form-input" placeholder="e.g. 8.5" value="${prefill.interestRate||''}" />
              </div>
              <div>
                <label class="form-label">Monthly EMI</label>
                <input id="liab_emi" type="number" step="0.01" class="form-input" placeholder="Monthly payment" value="${prefill.monthlyEmi||''}" />
              </div>
            </div>
            <div class="grid-2">
              <div>
                <label class="form-label">Start Date</label>
                <input id="liab_startDate" type="date" class="form-input" value="${prefill.startDate||''}" />
              </div>
              <div>
                <label class="form-label">Due Date</label>
                <input id="liab_dueDate" type="date" class="form-input" value="${prefill.dueDate||''}" />
              </div>
            </div>
            <div class="grid-2">
              <div>
                <label class="form-label">Principal Amount</label>
                <input id="liab_principal" type="number" step="0.01" class="form-input" placeholder="Original loan amount" value="${prefill.principal||''}" />
              </div>
              <div>
                <label class="form-label">Lender / Bank</label>
                <input id="liab_lender" class="form-input" placeholder="e.g. SBI, HDFC" value="${prefill.lender||''}" />
              </div>
            </div>
            <div class="grid-2">
              <div>
                <label class="form-label">Loan Account No.</label>
                <input id="liab_accountNo" class="form-input" placeholder="e.g. GL20250401" value="${prefill.accountNo||''}" />
              </div>
              <div>
                <label class="form-label">Collateral / Security</label>
                <input id="liab_collateral" class="form-input" placeholder="e.g. 22ct gold chain 25g" value="${prefill.collateral||''}" />
              </div>
            </div>
            <div>
              <label class="form-label">Notes</label>
              <textarea id="liab_notes" class="form-input form-textarea" rows="2" placeholder="Any additional details...">${prefill.notes||''}</textarea>
            </div>
            <div class="grid-2">
...
function openEditAllocationModal() {
  ...
  const fields = targets.map(t => `
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:8px;">
      ...
      <input type="number" min="0" max="100" class="form-input" style="width:80px;" value="${t.target}" data-cat="${t.category}" />
  ...
  const html = `
    <div class="modal-overlay show" id="editAllocOverlay">
      <div class="modal" style="max-width:420px;">
        ...
        <div class="modal-body">
          <div class="text-muted" style="font-size:12px;margin-bottom:16px;">Total should add up to 100%</div>
          <div id="allocFields">${fields}</div>
          <div id="allocTotal" style="font-size:13px;color:var(--text-2);margin:8px 0;"></div>
  ...
  document.querySelectorAll('#allocFields input').forEach(input => {
    input.oninput = () => {
...
async function saveAllocationTargets() {
  ...
  const newTargets = [...inputs].map(input => ({
    id: 'alloc_' + input.dataset.cat.replace(/\s/g,'').toLowerCase(),
        ...
        <div class="modal-body">
          <form id="sipForm" class="space-y-4">
            <div>
              ...
              <input id="sip_name" class="form-input" placeholder="e.g. Axis BC Fund" required />
            </div>
            <div class="grid-2">
              <div>
              ...
              <div>
                ...
                <input id="sip_amount" type="number" step="0.01" class="form-input" placeholder="₹" required />
              </div>
            </div>
            <div>
              <label class="form-label">Percentage of Budget (%)</label>
              <input id="sip_pct" type="number" step="0.1" class="form-input" placeholder="e.g. 30" />
            </div>
            <div class="grid-2">
