...
-- ─── 1. Quick Links Policies ─────────────────────────────────────────────
DROP POLICY IF EXISTS "ql_links_select" ON lm_quick_links;
CREATE POLICY "ql_links_select" ON lm_quick_links FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "ql_links_insert" ON lm_quick_links;
CREATE POLICY "ql_links_insert" ON lm_quick_links FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "ql_links_update" ON lm_quick_links;
CREATE POLICY "ql_links_update" ON lm_quick_links FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "ql_links_delete" ON lm_quick_links;
CREATE POLICY "ql_links_delete" ON lm_quick_links FOR DELETE USING (auth.uid() = user_id);


-- ─── 2. Quick Links Config Policies ──────────────────────────────────────
DROP POLICY IF EXISTS "ql_config_select" ON lm_ql_config;
CREATE POLICY "ql_config_select" ON lm_ql_config FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "ql_config_insert" ON lm_ql_config;
CREATE POLICY "ql_config_insert" ON lm_ql_config FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "ql_config_update" ON lm_ql_config;
CREATE POLICY "ql_config_update" ON lm_ql_config FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "ql_config_delete" ON lm_ql_config;
CREATE POLICY "ql_config_delete" ON lm_ql_config FOR DELETE USING (auth.uid() = user_id);


-- ─── 3. Study Sessions Policies ──────────────────────────────────────────
DROP POLICY IF EXISTS "users_own_sessions_select" ON study_sessions;
CREATE POLICY "users_own_sessions_select" ON study_sessions FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "users_own_sessions_insert" ON study_sessions;
CREATE POLICY "users_own_sessions_insert" ON study_sessions FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "users_own_sessions_update" ON study_sessions;
CREATE POLICY "users_own_sessions_update" ON study_sessions FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "users_own_sessions_delete" ON study_sessions;
CREATE POLICY "users_own_sessions_delete" ON study_sessions FOR DELETE USING (auth.uid() = user_id);